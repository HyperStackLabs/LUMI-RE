...
useEffect(() => {
  ...
  // Filter Logic
  const categories = ["All", "Chronograph", "Diver", "Dress", "Sport"];
      ...
      <div className={`sticky top-20 z-40 ${theme.bg} border-b ${theme.border} transition-colors duration-700`}>
         <div className="max-w-7xl mx-auto px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className={`hidden md:flex items-center gap-4 text-xs uppercase tracking-widest ${theme.subText}`}>
                <span>{filteredWatches.length} Results</span>
            ...
            >
              ...
              <div className="relative w-full overflow-hidden mb-6 bg-gray-100 dark:bg-[#111]">
                 ...
                 <div className="absolute bottom-0 left-0 w-full translate-y-full group-hover:translate-y-0 transition-transform duration-500 ease-in-out">
                    <button onClick={() => {localStorage.setItem('product-type', watch.type), route.push(`/product-details/${watch.id}`)}} className="w-full py-4 bg-[#D4AF37] text-black text-xs uppercase tracking-widest font-bold hover:bg-white transition-colors">
                        Add to Bag — ${watch.price.toLocaleString()}
                    </button>
                 </div>
              </div>
              ...
              <div className="text-center w-full relative">
                 ...
                 <div className="h-6 overflow-hidden relative w-full">
                    <div className={`absolute w-full transition-all duration-500 ${hoveredProduct === watch.id ? '-translate-y-full opacity-0' : 'translate-y-0 opacity-100'}`}>
                         <p className={`text-sm font-light ${theme.text}`}>${watch.price.toLocaleString()}</p>
                    </div>
                    <div className={`absolute w-full transition-all duration-500 ${hoveredProduct === watch.id ? 'translate-y-0 opacity-100' : 'translate-y-full opacity-0'}`}>
                         <p className="text-xs uppercase tracking-wider text-[#D4AF37]">{watch.specs}</p>
                    </div>
                 </div>
              </div>
